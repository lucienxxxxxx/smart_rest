...
import com.pitaya.smart_rest.guke.service.impl.OnlineMemberServiceImpl;
 ...
 */
@Controller
@RequestMapping("guke/onlineMember")
public class OnlineMemberController extends BaseController {

    @Autowired
    private OnlineMemberServiceImpl memberService;
    ...
    public ResultInfo addOrUpdate(HttpServletRequest request, Member member, Integer flag) {
        Integer userId = LoginUserUtil.releaseUserIdFromCookie(request);
        memberService.addOrUpdate(member,flag,userId);
        String msg = "添加成功";
        if (flag == 1) {
            msg = "修改成功";
        }
