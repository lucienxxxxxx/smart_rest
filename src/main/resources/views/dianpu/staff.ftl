            ...
            <div class="layui-form toolbar">
                <div class="layui-form-item">
                    ...
                    <div class="layui-inline">
                        ...
                        <div class="layui-input-inline mr0">
                            <input name="mobile" class="layui-input" type="text" placeholder="请输入电话号码"/>
                        </div>
                    </div>
                    <div class="layui-inline">
                        <label class="layui-form-label w-auto">备注:</label>
                        <div class="layui-input-inline mr0">
                            <input name="note" class="layui-input" type="text" placeholder="请输入备注"/>
                        </div>
                    </div>
                    <div class="layui-inline ">
...
<script type="text/html" id="tableBarUser">
    <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="swith">切换状态</a>
    ...
    layui.use(['layer', 'form', 'table', 'util', 'admin', 'formSelects', "xmSelect"], function () {
        ...
        table.on('tool(tableUser)', function (obj) {
            ...
            if (layEvent === 'edit') { // 修改
                ...
                doDel(data.id, data.tureName);
            }
            else if (layEvent === 'swith') { // 删除
        ...
        function showEditModel(mUser) {
            admin.open({
                type: 1,
                title: (mUser ? '修改' : '添加') + '员工',
                content: $('#modelUser').html(),
                success: function (layero, dIndex) {
                    $(layero).children('.layui-layer-content').css('overflow', 'visible');
                    var url = mUser ? '${ctx}/dianpu/staff/addOrUpdate?flag=1' : '${ctx}/dianpu/staff/addOrUpdate?flag=0';
                    // 回显数据
        ...
        function switchStatus(id) {
            admin.open({
                ...
                success: function (layero, dIndex) {
                    $(layero).children('.layui-layer-content').css('overflow', 'visible');
                    var url = '${ctx}/dianpu/staff/switchStatus';
                    // 回显数据
